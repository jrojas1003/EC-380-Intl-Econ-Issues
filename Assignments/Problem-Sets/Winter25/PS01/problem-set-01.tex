...
\begin{parts}
    \part[4] How is the Trade to GDP ratio measured?
    \vspace*{\stretch{1}}
    \part[4] How do people/labor flows impact trade integration? Why?
    \vspace*{\stretch{1}}
    \part[4] Have trade frictions \textbf{increased or decreased} over time? How does that impact trade flows? 
    \vspace*{\stretch{1}}
\end{parts} 
...
\begin{parts}
    \part[2] Which country has \textbf{comparative advantage in producing Bath Bombs}?
    \vspace*{2in}
    \part[2] What is the \textbf{price of Rubber Ducks} in each country?
    \vspace*{2in}
    \part[2] What are the \textbf{maximum quantities of each good} that Home and Foreign can produce? (Fill in the table) 
    \input{tables/Q2-table-b}
    \vspace*{0.5in}
    \newpage
    \part[4] Sketch the PPFs for \textbf{both countries} in a single graph. 
    Be sure to correctly label the graph for full credit. 
    \vspace*{\stretch{1}}
    \part[4] What should be the \textbf{World Price of Rubber Ducks}?  
    \vspace*{\stretch{1}}
    \part[4] Suppose Home prefers to consume \textbf{5 Rubber Ducks} for every \textbf{2 Bath Bombs}. 
